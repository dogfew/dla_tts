...
class WanDBWriter:
        ...
        self.wandb.log(
            {
                self._scalar_name(scalar_name): self.wandb.Audio(
                    audio, sample_rate=sample_rate, caption=caption
                )
            },
            step=self.step,
        )
    ...
    def add_images(self, scalar_name, images):
        # self.wandb.log(
        #     *[{self._scalar_name(scalar_name + '1'): self.wandb.Image(image)} for image in images], step=self.step
        # )
        # todo!
        self.wandb.log({"images": self.wandb.Image(images)}, step=self.step)
